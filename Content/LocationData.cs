...
public class Location
{
    ...
    /// <summary>
    /// Per-location pool of conversational dialogues. Each entry is a factory
    /// `(npcName, playerName) => Dialogue`. CommandParser.Talk() picks one at
    /// random when the player talks to a friendly NPC here. Empty pool falls
    /// back to DialogueData.Default so existing rooms still have flavor.
    /// </summary>
    public List<Func<string, string, Dialogue>> DialoguePool { get; set; } = new();
    ...
    public static Dictionary<string, Location> BuildWorld()
    {
        ...
        world["nar_shadaa_orbit"] = new Location
        {
            ...
            AmbientMessages = new()
            {
                ...
                "A swarm of pleasure yachts line the nightclub district, music leaking over comms channels.",
                ...
                "Two smugglers negotiate fuel prices on an unsecured channel. What type of fuel has \"just enough firepower for an exit\"? Metaphor, clearly.",
        ...
        world["nar_shadaa_rimmers_rest_bar"] = new Location
        {
            Id = "nar_shadaa_rimmers_rest_bar",
            Name = "Nar Shadaaa - Rimmer's Rest",
            Description = "This place has everything: a scenic view of the stars, bartender droids, a laconic Trandoshan with an itchy trigger finger.",
            Exits = new()
            {
                ["east"] = "nar_shadaa_landing_pad_5",
            },
            EncounterChance = 0.2,
            AmbientMessages = new()
            {
                "The dim lighting and corroded, green copper floor panels make this quite the place to hang low.",
                "A neon sign of lizard lights up flames. The signature cocktail, \"Newt Fireray\", a jab at the decades-old dispute between the Trade Federation and the Hutt Cartel.",
                "The oblong-headed droid at the bar scrubs the countertop.",
                "A middle-aged human in gray robes and a ochre yellow undershirt paces erratically. He's seen things.",
            },
            FriendlyNPCs = new() { NPCData.BartenderNarShadaa },
            DialoguePool = new() { DialogueData.NarShadaaaBar },
            PlanetName = "Nal Hutta",
            StarSystemName = "Tyrius System",
            SectorName = "Savareen Sector",
            TerritoryName = "Outer Rim Territory",
            Climate = Climate.Normal,
            HyperspaceCoordinates = new[] { 18, 16 },
        };

        world["nar_shadaa_landing_pad_5"] = new Location
        {
            Id = "nar_shadaa_landing_pad_5",
            Name = "Nar Shadaa - Landing Pad 5",
            Exits = new()
            {
                ["up"] = "nar_shadaa_orbit",
                ["west"] = "nar_shadaa_rimmers_rest_bar",
            },
            EncounterChance = 0.2,
            AmbientMessages = new()
            {
                "A massive freighter with trapezoidal side panels lifts off.",
                "The neon city extends nearly kilometers deep.",
                "A Rodian in a white-and-yellow flight suits flicks a death stick off the platform. She's watching you.",
            },
            FriendlyNPCs = new(),
            DialoguePool = new(),
            PlanetName = "Nal Hutta",
